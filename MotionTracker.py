...
class MotionTrackerManager():
    ...
    def __init__(self, *, rec=False, party=False, fig=False, bgv=True):
        
        self.width_corr, self.height_corr = [0, 0], [0, 0]

        self.rec, self.party, self.fig, self.bgv = rec, party, fig, bgv
    ...
    def run(self):
        while True:
            ...
            if self.rec:
                self.videoRecorder.record_video(frame=frame, diff=diff)
            if self.fig:
                self.figure.update_figure(frame_diff=frame_diff)
            if self.party:
            ...
            # self.render_text(frame=frame, text=diff)
...
if __name__ == "__main__":
    ...
    party = True if str(sys.argv[2]) == "-p" else False
    fig = True if str(sys.argv[3]) == "-f" else False

    motionTrackerManager = MotionTrackerManager(rec=rec,
                                                party=party,
                                                fig=fig)
